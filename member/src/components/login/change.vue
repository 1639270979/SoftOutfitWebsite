<template>
	<div id="register">
		...
		<div class="btn"><span @click="registerFun">修改密码</span></div>
	</div>
</template>
<script>
	import { getVf,changePsw } from '../../common/axios/api.js';
  let Base64 = require("js-base64").Base64//还是require
	export default {
		...
		methods: {
			obtainFun: function() {
				if (this.status == 1 && /^1[3|4|5|6|7|8|9]{1}\d{9}$/.test(this.tel)) {
					...
					let obj = {'mobile':this.tel,'type':0};
					getVf(obj).then(data => {
						if (!data.status) {
        ...
        this.pw = Base64.encode(this.psw); //还是那些操作
				...
				changePsw(obj).then(data => {
					if (data.status) {
						this.txtPromptFun('修改成功');
						setTimeout(()=>{

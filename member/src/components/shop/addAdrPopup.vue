<template>
	<div id="addAdrPopup" v-if="isShowAdd">
		<div class="popout">
			<div class="title">
			...
			<div class="edit">
				<ul>
					...
					<li>
						...
						<div class="address">
							<input id="adr" name="area" class="region-btn" type="text" readonly placeholder="点击选择" :vaule="adr"/>
...
<script>
	...
	export default {
		props: ['isShowAdd','address','type','aid'],
		data() {
			return {
				province : [],
				city     : {},
		...
		methods: {
			...
			saveFun: function() {
				...
				if (!(/\S/.test(this.name))) {
					this.txtFun('姓名不能留空');
				}else if(!(/^[a-zA-Z\s\u4e00-\u9fa5]{2,16}$/.test(this.name))){
          this.txtFun('姓名为2-16位且不能为数字');
        }else if (!(/^1[3|4|5|6|7|8|9]{1}\d{9}$/.test(this.tel))) {
					this.txtFun('输入正确的手机号码');
				}else if (adrVal === '') {
					this.txtFun('地区不能留空');
				}else if (!(/\S/.test(this.detailed))) {
					this.txtFun('详细地址不能留空');
				}else {
					if (this.aid) {//编辑

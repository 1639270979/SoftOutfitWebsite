<template>
	<div id="openVIPInfo">
		...
		<div class="info">
			<ul>
				...
				<li>
					...
					<div class="inp">{{ info.mobile }}</div>
				</li>
				<li>
					<span class="txt">身份证号</span>
					<div class="inp"><input type="text" v-model.trim="idcard_no"></div>
				</li>
				<li>
			...
			<div class="photo">
				<p>请上传您的身份证证件信息</p>
				<div id="box">
					<div class="img-btn" id="idcard_front_btn"><img src="./image/positive.jpg" id="idcard_front"></div>
					<div class="img-btn" id="idcard_back_btn"><img src="./image/back.jpg"id="idcard_back"></div>
				</div>
			</div>
		</div>
		<div class="read">
		...
		<div class="bottom-btn" :class="checkedNames.length != 0 ? 'action' : ''" @click="openFun">立即开通</div>
...
<script>
	...
	import { vipInfo,saveVip,areaJson } from '../../common/axios/api.js';
		...
		methods: {
			...
			ajaxFun: function() {
				vipInfo().then(data => {
					...
					this.idcard_frontImg();
					this.idcard_backImg();
				});
			},
			idcard_frontImg: function() {//七牛云
				var this_ = this;
				var upToken = this.token;
				Qiniu.uploader({
					runtimes: 'html5,flash,html4',				// 上传模式，依次退化
					browse_button: 'idcard_front_btn',					// 上传选择的点选按钮，必需
				    ...
				    init: {
						...
						'FileUploaded': function(up, file, info) {
							...
							document.getElementById('idcard_front').setAttribute('src',url);
							this_.idcard_front = url;
						},
			...
			idcard_backImg: function() {//七牛云
				var this_ = this;
				var upToken = this.token;
				Qiniu.uploader({
					runtimes: 'html5,flash,html4',				// 上传模式，依次退化
					browse_button: 'idcard_back_btn',					// 上传选择的点选按钮，必需
					uptoken : upToken,							// uptoken是上传凭证，由其他程序生成
					get_new_uptoken: false,						// 设置上传文件的时候是否每次都重新获取新的uptoken
					domain: 'http://public.idmei.cn/',			// bucket域名，下载资源时用到，必需
					container: 'box',							// 上传区域DOM ID，默认是browser_button的父元素
				    ...
				    init: {
						...
						'FileUploaded': function(up, file, info) {
							...
							document.getElementById('idcard_back').setAttribute('src',url);
							this_.idcard_back = url;
						},
						'UploadComplete': function(up, file) {
						},
						'Error': function(up, err, errTip) {
            ...
            this.alertPrompt('身份证号格式有误');
          }else if (document.getElementById('adrVal').value == '') {
						this.alertPrompt('请选择城市')
					}else if (this.idcard_front == '') {
						this.alertPrompt('请上传身份证正面照')
					}else if (this.idcard_back == '') {
						this.alertPrompt('请上传身份证反面照')
					}else {
						let obj = {'name': this.name,'idcard_no': this.idcard_no,'aid': document.getElementById('adrVal').value,'idcard_front': this.idcard_front,'idcard_back': this.idcard_back,'vip_type': this.vip_type};
			...
			.user-info {
				...
				.name {
					...
					span {
						...
						.rem(margin-top,3);
						.rem(margin-left,30);
						background: url(./image/vip-icon_1.png) no-repeat;

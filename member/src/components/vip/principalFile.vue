<template>
	<div id="principalFile">
		<p class="p title">装修主材套餐包	</p>
		...
		<div class="box">
			<a class="d1" v-if="wxSystem" :href="'http://member.idmei.com'+caseInfo.material"><span>查看文件<i></i></span><div><p>{{ caseInfo.material_name }}</p></div></a>
			<a class="d1" v-else :href="'/member/pdf?http://member.idmei.com'+caseInfo.material"><span>查看文件<i></i></span><div><p>{{ caseInfo.material_name }}</p></div></a>
		</div>
		<p class="p2">也可前往大美官网www.idmei.com凭下载码 <span>{{ caseInfo.download_code }}</span> 下载以上文件至电脑。</p>
		<div class="advertisement">
			<p>认证联盟单位</p>
			<div class="img"><img src="./image/dj_logo.jpg"></div>
		</div>
	</div>
</template>
<script>

<template>
	<div id="softOutfitDesign">
		<p class="title">提交软装设计需求</p>
		<ul>
			<li class="inp">
			...
			<li class="inp">
				<span class="txt"><i>*</i>期望软装价格</span>
				<div class="choice" @click="pleasePopupShow('expectPrice')">{{ expect_price }}<i class="icon-right"></i></div>
			</li>
			<li class="inp">
				<span class="txt"><i>*</i>期望软装空间</span>
				<div class="choice" @click="pleasePopupShow('expectSpace')">{{ space_arr }}<i class="icon-right"></i></div>
			</li>
			<li class="inp">
				<p class="p1">期望软装风格的表述</p>
			</li>
			<li class="textarea">
			...
			<li class="pano">
				<div class="left">全景效果图<i class="icon-doubt" @click="isdoubt = true"></i></div>
				<div class="right">
					<span :class="need_pano == 2 ? 'action' : ''" @click="need_pano = 2">不需要</span>
					<span :class="need_pano == 1 ? 'action' : ''" @click="need_pano = 1">需要</span>
				</div>
			</li>
		</ul>
		<div class="prompt">
		...
		<!-- <p class="pano-money" v-if="need_pano == 1">全景效果图需补差价<span>¥10000</span></p> -->
		<div class="bottom-btn">
			<span class="close" @click="close">取消</span><span @click="submitFun">提交给VIP会员专属客服</span>
		</div>
		<div class="doubt-mask" v-show="isdoubt">
			<div class="popout">
				<div class="cancel"><span class="icon" @click="isdoubt = false"></span></div>
				<div class="body">
					<div class="left">
		...
		<div class="money-popout" v-if="isDoubt">
			<div class="mask">
	...
	export default {
		data() {
			return {
				...
				statusObj  : {
					acreage: 1, //房屋面积
					expectPrice: 1,	//期望软装价格
					expectSpace: [1,2,3,4,5,6,7,8,9]	//期望软装空间
				},
				isPlane: true,
				planer_graph: '',
				expect_style: '',
		...
		computed: {
			...
			space_arr: function() { //期望软装空间
				let arr = this.statusObj.expectSpace;
				let str = [];
				let expectSpace = ['客厅','主卧','次卧','卧室','儿童房','书房','厨房','餐厅','客房'];
				if (arr.length >= 6) {
					for (var i = 0; i < arr.length; i++) {
						if (i < 5) {
							str.push(expectSpace[i]);
						}
					}
					return str.join('，')+'...';
				}else {
